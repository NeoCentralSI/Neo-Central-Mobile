...
class _DefenceRevisionPanelState extends State<DefenceRevisionPanel>
  ...
  bool get _isFinalized => _board?.isFinalized ?? false;
  ...
  Future<void> _load() async {
    ...
    try {
      final board = await _api.getRevisions(widget.defenceId);
      if (!mounted) return;
  ...
  Future<void> _editAction(RevisionItem revision) async {
    ...
    await _runRevisionAction(
      ...
      () => _api.updateRevision(
        widget.defenceId,
        revision.id,
        action: DefenceRevisionAction.saveAction,
  ...
  Future<void> _submit(RevisionItem revision) async {
    await _runRevisionAction(
      ...
      () => _api.updateRevision(
        widget.defenceId,
        revision.id,
        action: DefenceRevisionAction.submit,
      ),
  ...
  Future<void> _cancelSubmit(RevisionItem revision) async {
    await _runRevisionAction(
      ...
      () => _api.updateRevision(
        widget.defenceId,
        revision.id,
        action: DefenceRevisionAction.cancelSubmit,
      ),
    ...
    await _runRevisionAction(
      ...
      () => _api.updateRevision(
        ...
        action: revision.isFinished
            ? DefenceRevisionAction.unapprove
            : DefenceRevisionAction.approve,
      ),
      revision.isFinished
  ...
  Future<void> _delete(RevisionItem revision) async {
    ...
    await _runRevisionAction(
      revision,
      () => _api.deleteRevision(widget.defenceId, revision.id),
  ...
  Future<void> _create() async {
    ...
    try {
      await _api.createRevision(
        widget.defenceId,
        defenceExaminerId: draft.examinerId,
  ...
  Future<void> _toggleFinalization() async {
    ...
    try {
      if (finalize) {
        await _api.finalizeRevisions(widget.defenceId);
      } else {
        await _api.unfinalizeRevisions(widget.defenceId);
      }
      if (!mounted) return;
      _showMessage(
        finalize
...
class _SummaryCard extends StatelessWidget {
  final DefenceRevisionBoard board;
