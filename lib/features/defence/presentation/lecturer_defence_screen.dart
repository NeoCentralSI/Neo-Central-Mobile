...
/// Sidang TA — lecturer (and HoD acting as lecturer) view.
///
/// Two tabs:
///   • Mahasiswa Bimbingan — defences where the user is a supervisor
///   • Menguji Mahasiswa   — defences where the user is an examiner
///
/// FCM notifications of type `defence_examiner_assigned` deep-link straight
...
class _SupervisedTabState extends State<_SupervisedTab>
  ...
  Future<void> _fetch() async {
    ...
    try {
      final list = await _api.getSupervisedStudentDefences();
      if (!mounted) return;
      setState(() {
        _items = list;
        _isLoading = false;
  ...
  Future<void> _openDetail(Map<String, dynamic> item) async {
    ...
    await Navigator.of(context).push(MaterialPageRoute(
      builder: (_) =>
          DefenceDetailScreen(defenceId: id, user: widget.user),
    ));
    if (mounted) _fetch();
  }
...
class _ExaminerRequestsTabState extends State<_ExaminerRequestsTab>
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    // Match web priority sort: pending first, then available, then unavailable.
  ...
  Future<void> _openResponseDialog(Map<String, dynamic> item) async {
    final result =
        await showDefenceExaminerResponseDialog(context, defence: item);
    if (result == true) {
      await _fetch();
    }
  }
          ...
          child: _isLoading
              ...
              : _error != null
                  ...
                  : _DefenceList(
                      ...
                      onRefresh: _fetch,
                      emptyText: _items.isEmpty
                          ? 'Belum ada penugasan menguji sidang TA.'
                          : 'Tidak ada hasil yang cocok.',
        ...
        children: [
          Row(
            ...
            children: [
              ...
              _DefenceStatusBadge(status: status),
            ],
          ),
          const SizedBox(height: 8),
          Text(
...
class _DefenceStatusBadge extends StatelessWidget {
  final String status;
  const _DefenceStatusBadge({required this.status});

  @override
  Widget build(BuildContext context) {
    return AppBadge(label: _label(status), variant: _variant(status));
  }
    ...
    switch (status) {
      case 'available':
        return const AppBadge(
            label: 'Disetujui', variant: BadgeVariant.success);
      case 'unavailable':
        return const AppBadge(
            label: 'Ditolak', variant: BadgeVariant.destructive);
      case 'pending':
      default:
        return const AppBadge(
            label: 'Menunggu Respons', variant: BadgeVariant.warning);
    }
  }
}
...
class _DefenceList extends StatelessWidget {
