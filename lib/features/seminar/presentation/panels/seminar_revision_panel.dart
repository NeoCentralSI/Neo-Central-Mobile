...
/// Revisi panel — supervisor view of revision items.
///
/// Mobile scope: supervisor can approve / unapprove submitted revisions
/// and finalize / unfinalize the board. Students don't access the detail
/// screen on mobile (entry is from lecturer/HoD cards only), so the
/// student-side create/edit/submit flow is intentionally omitted.
...
class _SeminarRevisionPanelState extends State<SeminarRevisionPanel>
  ...
  Widget build(BuildContext context) {
    ...
    // Filter: supervisor only sees submitted/finished items
    final allRevisions = ((_board['revisions'] as List?) ?? const [])
        .whereType<Map>()
        .map((m) => Map<String, dynamic>.from(m))
        .toList();
    final visible = allRevisions
        .where((r) => r['studentSubmittedAt'] != null || r['isFinished'] == true)
        .toList()
      ..sort((a, b) =>
    ...
    final allApproved =
        allRevisions.every((r) => r['studentSubmittedAt'] == null || r['isFinished'] == true);
    final canFinalize =
    ...
    return Row(
      children: [
        ...
        if (_isSupervisor) ...[
          if (_isFinalized)
            OutlinedButton.icon(
              onPressed: _busyFinalize ? null : () => _finalize(finalize: false),
              icon: _busyFinalize
                  ? const SizedBox(
                      width: 14,
                      height: 14,
