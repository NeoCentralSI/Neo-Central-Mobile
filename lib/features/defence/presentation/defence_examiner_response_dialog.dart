...
Future<bool?> showDefenceExaminerResponseDialog(
  ...
  required LecturerDefenceListItem defence,
}) {
...
class _DefenceExaminerResponseDialog extends StatefulWidget {
  final LecturerDefenceListItem defence;
...
class _DefenceExaminerResponseDialogState
  ...
  DefenceExaminerResponse? _submitting;
  ...
  Future<void> _respond(DefenceExaminerResponse response) async {
    final examinerId = widget.defence.myExaminerId;
    ...
    try {
      final result = await _api.respondToExaminerAssignment(
        ...
        examinerId,
        response: response,
        unavailableReasons: response == DefenceExaminerResponse.unavailable
      ...
      final accepted = response == DefenceExaminerResponse.available;
      final message = accepted
          ? result.defenceTransitioned
                ? 'Penugasan disetujui. Semua penguji telah bersedia dan sidang siap dijadwalkan.'
  ...
  Widget build(BuildContext context) {
    final defence = widget.defence;
    final supervisors = [...defence.supervisors]
                ...
                child: Column(
                  ...
                  children: [
                    _InfoLine(
                      ...
                      subtitle: defence.studentNim,
                    ),
                    const SizedBox(height: 10),
                    _InfoLine(
                      icon: Icons.menu_book_outlined,
                      title: defence.thesisTitle,
                    ),
              ...
              Row(
                children: [
                  ...
                  Chip(
                    ...
                    label: Text('Penguji ${defence.myExaminerOrder ?? '-'}'),
                  ),
                ],
              ),
              const SizedBox(height: 8),
              Text(
                'Apakah Anda bersedia menjadi penguji untuk sidang tugas akhir mahasiswa ini?',
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: _submitting == null
                          ? () => _respond(DefenceExaminerResponse.unavailable)
                          : null,
                      icon: _ActionIcon(
                        active:
                            _submitting == DefenceExaminerResponse.unavailable,
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: _submitting == null
                          ? () => _respond(DefenceExaminerResponse.available)
                          : null,
                      icon: _ActionIcon(
                        active:
                            _submitting == DefenceExaminerResponse.available,
