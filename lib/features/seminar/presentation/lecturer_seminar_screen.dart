...
class _LecturerListTabState extends State<_LecturerListTab>
  ...
  List<LecturerSeminarListItem> _items = const [];
  ...
  Future<void> _load() async {
    ...
    try {
      final result = widget.examinerView
          ...
          : await _api.getSupervisedStudentSeminars();
      if (!mounted) return;
  ...
  List<LecturerSeminarListItem> get _filteredItems {
  ...
  Future<void> _openDetail(LecturerSeminarListItem item) async {
  ...
  Future<void> _respond(LecturerSeminarListItem item) async {
    final changed = await showExaminerResponseDialog(context, seminar: item);
      ...
      return _EmptyList(
        ...
        message: _items.isEmpty
            ? widget.examinerView
                  ...
                  : 'Belum ada mahasiswa bimbingan yang mendaftar seminar hasil.'
...
class _SeminarCard extends StatelessWidget {
  final LecturerSeminarListItem item;
  final bool examinerView;
          ...
          Row(
            ...
            children: [
              ...
              if (examinerView)
                ...
                AppBadge(
                  label: seminarStatusLabel(item.status.value),
                  variant: seminarStatusVariant(item.status.value),
                ),
            ],
          ),
          const SizedBox(height: 8),
