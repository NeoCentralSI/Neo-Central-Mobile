...
/// Penilaian panel — examiner assessment form + supervisor/finalised rekap.
///
/// Mobile scope:
///   • Examiner (ongoing): inline form to fill scores + revisionNotes + Submit/Save Draf.
///   • Supervisor (ongoing): rekap matrix + "Tetapkan Hasil Seminar" with revision toggle.
///   • Finalised (any role): rekap matrix only.
class SeminarAssessmentPanel extends StatefulWidget {
  ...
  @override
  State<SeminarAssessmentPanel> createState() =>
      _SeminarAssessmentPanelState();
}
...
class _SeminarAssessmentPanelState extends State<SeminarAssessmentPanel>
  ...
  // Examiner assessment form data
  Map<String, dynamic>? _form;
  final Map<String, int> _scores = {};
  final TextEditingController _notesCtrl = TextEditingController();
  String? _submittingMode; // 'draft' | 'submit'

  // Finalization data (supervisor / read-only rekap)
  ...
  Future<void> _fetch() async {
    ...
    try {
      // Always load finalization rekap; examiner form only when needed.
      final finalData = await _api.getFinalizationData(widget.seminarId);

      Map<String, dynamic>? form;
      if (_isUserExaminer && !_isFinalized) {
        try {
          form = await _api.getAssessment(widget.seminarId);
        } catch (_) {
          form = null;
        }
      }
  ...
  void _hydrateScoresFromForm() {
    ...
    final notes = form['examiner']?['revisionNotes']?.toString() ?? '';
  ...
  Future<void> _submitAssessment({required bool isDraft}) async {
    ...
    try {
      ...
      await _api.submitAssessment(
        ...
        revisionNotes: _notesCtrl.text.trim(),
        isDraft: isDraft,
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _finalize() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: const Text('Tetapkan Hasil Seminar?'),
        content: const Text(
          'Tindakan ini akan menetapkan hasil akhir seminar secara '
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Hasil seminar berhasil ditetapkan.'),
          backgroundColor: AppColors.successDark,
          behavior: SnackBarBehavior.floating,
        ),
      );
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            if (_isUserExaminer && !_isFinalized && _form != null)
              _buildExaminerForm(),
            if (_isUserExaminer && !_isFinalized && _form != null)
  ...
  Widget _buildExaminerForm() {
    final form = _form!;
    final examiner = (form['examiner'] as Map?) ?? const {};
    final isSubmitted = (examiner['assessmentSubmittedAt'] ?? '').toString().isNotEmpty;
    final groups = ((form['criteriaGroups'] as List?) ?? const [])
        .whereType<Map>()
    ...
    return AppCard(
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text('Form Penilaian', style: AppTextStyles.label),
                ),
                AppBadge(
          ...
          for (final group in uniqueGroups) _buildGroupSection(group, isSubmitted),
          // Notes
          Padding(
  ...
  Widget _buildRekap() {
    ...
    final examiners = ((data['examiners'] as List?) ?? const [])
        ...
        .map((m) => Map<String, dynamic>.from(m))
        .toList();
    final seminar = (data['seminar'] as Map?) ?? const {};
    final isFinalized = (seminar['resultFinalizedAt'] ?? '').toString().isNotEmpty;
    final avgScore = (data['averageScore'] as num?)?.toDouble();
    final canFinalize = data['recommendationUnlocked'] == true &&
        _isUserSupervisor &&
        !isFinalized;
    ...
    return Column(
      ...
      children: [
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child:
                          Text('Rekap Penilaian', style: AppTextStyles.label),
                    ),
                  ],
                ),
              ),
              if (examiners.isEmpty)
                ...
                for (final ex in examiners) _ExaminerSummaryCard(examiner: ex),
              ],
              if (avgScore != null) ...[
                Container(
                  padding: const EdgeInsets.all(14),
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      AppBadge(
                        label: avgScore >= 55 ? 'LULUS' : 'TIDAK LULUS',
                        variant: avgScore >= 55
                            ? BadgeVariant.success
        ...
        if (canFinalize) ...[
          const SizedBox(height: AppSpacing.base),
          _buildFinalizationCard(avgScore),
        ],
        if (isFinalized) ...[
          const SizedBox(height: AppSpacing.base),
          _buildFinalizedCard(seminar),
        ],
        if (!isFinalized &&
          ...
          Container(
            ...
            child: Text(
              'Penetapan hasil akan terbuka setelah seluruh penguji '
    ...
    return AppCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            'Berdasarkan akumulasi nilai rata-rata penguji.',
            style: AppTextStyles.caption
                .copyWith(color: AppColors.textSecondary),
          ),
          if (isPass) ...[
              ...
              children: [
                Checkbox(
                  ...
                  onChanged: _finalizing
                      ? null
                      : (v) => setState(() => _recommendRevision = v ?? false),
                ),
                const Expanded(
                  child: Text(
                    'Lulus dengan Revisi',
            ...
            Padding(
              ...
              child: Text(
                'Mahasiswa wajib melakukan revisi sebelum mendaftar sidang.',
                style: AppTextStyles.caption
                    .copyWith(color: AppColors.textSecondary),
              ),
            ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('Tetapkan Hasil Seminar'),
              style: ElevatedButton.styleFrom(
  ...
  Widget _buildFinalizedCard(Map seminar) {
    final status = (seminar['status'] ?? '').toString();
    final by = seminar['resultFinalizedBy']?.toString();
    final at = seminar['resultFinalizedAt']?.toString();
    String label;
    ...
    return AppCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: Text('Hasil Seminar', style: AppTextStyles.label)),
